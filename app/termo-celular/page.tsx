...
export default function TermoCelular() {
  const [formData, setFormData] = useState({
    ...
    numSerie: "",
    outrosItens: "",
    filial: "criciuma", // Add default branch
  ...
  const isFormValid = Boolean(formData.nomeFunc && formData.cpfFunc && formData.marca && formData.modelo && formData.numSerie)
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Calculate logo dimensions (11.83cm x 3.78cm)
      const logoWidth = 118.3
      const logoHeight = 37.8
      const logoX = (pageWidth - logoWidth) / 2

      // Add logo to PDF - starting exactly at the top margin
      ...
      pdf.setFontSize(14)
      const title = "TERMO DE RESPONSABILIDADE PELO USO DO CELULAR DA"
      ...
      // Equipment list
      pdf.setFont("cambria", "bold")
      pdf.text(`- Celular ${formData.marca} ${formData.modelo}, N° de Série: ${formData.numSerie || "___________________"}`, marginLeft, yPos)
      pdf.setFont("cambria", "normal")
      yPos += lineSpacing
      pdf.text("- Carregador", marginLeft, yPos)
      yPos += lineSpacing
      pdf.text("- Cabo USB", marginLeft, yPos)
      yPos += lineSpacing
      ...
      // Save PDF
      pdf.save("termo-responsabilidade-celular.pdf")
    } catch (error) {
  ...
  return (
    <main className="container mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold text-center mb-8">Sistema de Edição de Termo de Responsabilidade - Celular</h1>
      <Link href="/">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Cell phone specific fields */}
            <div className="space-y-2">
              <Label htmlFor="marca">Marca do Celular</Label>
              <Input
                id="marca"
                name="marca"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={handleChange}
                placeholder="Ex: Capa de proteção, fone de ouvido, etc"
              />
            </div>
        ...
        <Card>
          ...
          <CardContent className="bg-gray-50 p-6 rounded-md text-sm max-h-[750px] overflow-y-auto">
            <div className="space-y-6 font-serif" ref={documentRef}>
              ...
              <div className="text-center font-bold text-lg uppercase mb-6">
                TERMO DE RESPONSABILIDADE PELO USO DO CELULAR DA
                <br />
                EMPRESA
              </div>

              <p className="mb-6 text-justify">
                Pelo presente Termo de Responsabilidade pelo Uso de Equipamento do Celular da Empresa, de um
                ...
                {branches.find(b => b.id === formData.filial)?.cnpj || branches[0].cnpj}, neste ato
                representada pelo Senhor Alexandro Willemann da Silva, doravante denominada "EMPRESA" e de outro lado,{" "}
                <strong>{(formData.nomeFunc || "NOME DO FUNCIONÁRIO").toUpperCase()}</strong>, portador do CPF nº{" "}
                {formData.cpfFunc || "000.000.000-00"},
                doravante denominado FUNCIONÁRIO, tem entre si justo e contratado o que segue:
              </p>
              ...
              <div className="mb-6">
                ...
                <p className="mb-1">
                  - Celular {formData.marca} {formData.modelo}, N° de Série: <strong>{formData.numSerie || "___________________"}</strong>
                </p>
                <p className="mb-1">- Carregador</p>
                <p className="mb-1">- Cabo USB</p>
